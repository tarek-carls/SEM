...
• interpretation at the within/between level: construct explains the covariances
of the within/between part of its indicators
• the configural construct (at the between level) represents the aggregate of
...
• the cluster itself is not seen as the source/reason for variability of an individual construct
• therefore, between-cluster loadings are fixed to be the same as within-cluster
loadings (cross-level measurement invariance)
